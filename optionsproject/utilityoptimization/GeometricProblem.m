classdef GeometricProblem < handle
    ...
    properties        
        ...
        u
        %a=[ao;ac]
        %ao is vector of coefficients of x in the objective function (log(sum(ck*exp(ao \dot x))))
        ao
        ac
...
%            assert( sum(isfinite(b))==size(b,1));% No NaN values
        ...
        function [objective, x, res] = optimize(o) 
            %This is to solve the problem where c is near zero which is not
            %permitted by the Mosek syntax
            for i=1:length(o.u)
                if(o.u(i)>0)
                    o.ac(i,:)=o.ac(i,:)./o.u(i);
                    o.u(i)=1;
